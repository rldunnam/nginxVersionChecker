...
A Python script that monitors the NGINX download page for new stable versions and sends automated notifications via email and/or Slack when updates are detected.

## Features

- 🔍 Automatically checks for new NGINX stable versions
...
| `VERSION_FILE` | Path to version tracking file | nginx_last_version.txt | No |
...
EMAIL_FROM=nginx-alerts@yourcompany.com
...
```bash
python nginx_version_checker.py --enable-email
```

**Enable Slack notifications:**
```bash
python nginx_version_checker.py --enable-slack
```

**Enable both:**
```bash
python nginx_version_checker.py --enable-email --enable-slack
```
...
```bash
python nginx_version_checker.py \
  --enable-email \
...
```bash
python nginx_version_checker.py --dry-run --enable-email --enable-slack
```

**Verbose logging:**
```bash
python nginx_version_checker.py --verbose --enable-email
```

**Test with dry-run and verbose:**
```bash
python nginx_version_checker.py --dry-run --verbose --enable-email --enable-slack
```

### Get Help

```bash
python nginx_version_checker.py --help
```
...
0 */6 * * * cd /path/to/script && /usr/bin/python3 nginx_version_checker.py >> /var/log/nginx_checker.log 2>&1
```
...
# Add the cron job
0 */6 * * * /usr/bin/python3 /path/to/nginx_version_checker.py >> /var/log/nginx_checker.log 2>&1
```
...
# Secure it
chmod 600 ~/.nginx_credentials

# In crontab
0 */6 * * * . ~/.nginx_credentials && /usr/bin/python3 /path/to/nginx_version_checker.py >> /var/log/nginx_checker.log 2>&1
```
...
- Detect the current stable version
...
chmod 755 nginx_version_checker.py
```
...
```bash
# Redirect to file
python nginx_version_checker.py --enable-email >> nginx.log 2>&1

# View logs in real-time
tail -f nginx.log

# With cron, specify log file in crontab
0 */6 * * * /usr/bin/python3 /path/to/script.py >> /var/log/nginx_checker.log 2>&1
```
...
result = subprocess.run(
    ['python3', 'nginx_version_checker.py'],
    env=env,
    capture_output=True
)
...
```bash
python nginx_version_checker.py \
  --version-file /custom/path/nginx_version.txt \
  --enable-email
```
...
```bash
# Infrastructure team
EMAIL_TO=infra-team@company.com VERSION_FILE=nginx_version_infra.txt \
python nginx_version_checker.py --enable-email

# Security team
EMAIL_TO=security-team@company.com VERSION_FILE=nginx_version_security.txt \
python nginx_version_checker.py --enable-email
```

### Integration with CI/CD

Check for new NGINX versions before deploying:
...
NGINX uses a three-part version number: `MAJOR.MINOR.PATCH`
- Example: `1.24.0`

### Release Cycles

- **Stable branches** are released approximately every 1-2 years
- **Patch updates** within a stable branch are released as needed for bug fixes and security updates
- This script tracks stable releases to help you stay current with production-ready versions

### Why Monitor Stable Versions?

- Security patches and critical bug fixes
- Performance improvements
- New features in stable releases
- Compliance requirements for up-to-date software
- Advance planning for upgrade windows
...
- Improved HTML parsing reliability
- Added comprehensive documentation

### Version 1.0
- Initial release
- Basic version checking from NGINX download page
